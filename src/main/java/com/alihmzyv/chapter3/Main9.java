...
//after first send is called, linger.ms is started.
//when reached, the batch is sent to the brokers. There can be messages for different brokers, all are sent as
//separate requests, but at the same time
//add case where the first 10 messages are to the same partition, and the next 10 are to the other partition for both topics
//linger.ms is calculated per topic partition. if it is reached, then if needed multiple requests are sent
public class Main9 {
...
//        createTopicIfNotPresent("topic_2", partitions, replicationFactor);
        ...
        IntStream.rangeClosed(11, 20)
                .forEach(num -> {
                    int partition = 1;
                    ProducerRecord<String, String> record1 =
                            new ProducerRecord<>("topic_1", 1, null, String.format("message %s", num));
                    send(record1);
                    ProducerRecord<String, String> record2 =
                            new ProducerRecord<>("topic_2", 0, null, String.format("message %s", num));
                    send(record2);
                    try {
                        Thread.sleep(5_000);
